...
export function WindowControls({ closeOnly = false }: Props) {
  ...
  useEffect(() => {
    ...
    void w.onResized(() => { void w.isMaximized().then(setMaximized); }).then((un) => { unlisten = un; });
    return () => unlisten?.();
  }, [closeOnly]);
    ...
    <div className="flex h-full shrink-0 items-center gap-0.5 pr-1">
      {!closeOnly && (
        <>
          ...
          <CtlButton ariaLabel={maximized ? "Restore" : "Maximize"} onClick={() => void w.toggleMaximize()}>
...
function CtlButton({ ariaLabel, onClick, children, danger }: { ariaLabel: string; onClick: () => void; children: React.ReactNode; danger?: boolean }) {
  return (
    <button type="button" aria-label={ariaLabel} title={ariaLabel} onClick={onClick}
      className={cn(
