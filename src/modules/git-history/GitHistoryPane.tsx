...
export function GitHistoryPane({
  ...
  const loadInitial = useCallback(async () => {
    ...
    setEndReached(false)
    try {
      const entries = await invoke<GitLogEntry[]>("git_log_detail", {
        path: repoRoot,
        maxCount: PAGE_SIZE,
      })
  ...
  useEffect(() => {
    let cancelled = false
    invoke<string | null>("git_remote_url", { path: repoRoot })
      .then((url) => {
        if (cancelled) return
  ...
  const fetchFiles = useCallback(
    async (sha: string) => {
      ...
      bumpFiles()
      try {
        const files = await invoke<GitCommitFileChange[]>("git_commit_files", {
          path: repoRoot,
          sha,
        })
    ...
    <div className="flex max-h-[60vh] min-h-0 flex-col">
      <div className="shrink-0 border-b border-border/45 p-3">
        ...
        <div className="mt-2 flex min-w-0 items-center gap-1.5 text-[10.5px] text-muted-foreground">
          ...
          {commit.authorEmail ? (
            <>
              <span className="text-muted-foreground/45">&middot;</span>
...
const FileRow = memo(function FileRow({
  ...
  return (
    ...
    >
      {iconUrl ? (
        <span className="size-3.5 shrink-0 text-center text-[10px] leading-none">
          {iconUrl}
        </span>
      ) : (
