...
type PortPreset = { port: number; label: string; hint: string };

const PORT_PRESETS: PortPreset[] = [
  { port: 5173, label: "Vite", hint: "vite, sveltekit" },
  { port: 5174, label: "Vite (alt)", hint: "instance vite kedua" },
  { port: 3000, label: "Next.js", hint: "next, express, rails" },
  { port: 3001, label: "Next.js (alt)", hint: "instance next kedua" },
...
type PreviewAddressBarProps = { url: string; onSubmit: (url: string) => void; onReload: () => void };

export function PreviewAddressBar({ url, onSubmit, onReload }: PreviewAddressBarProps) {
  ...
  const submit = () => {
    ...
    if (!next) { setNotice("Masukkan URL atau pilih port."); return; }
    setNotice(null);
  ...
  const tryPort = async (port: number) => {
    ...
    if (!ok) { setNotice(`Tidak ada server di :${port}.`); return; }
    setDraft(targetUrl);
    onSubmit(targetUrl);
  };
  ...
  return (
    <div className="shrink-0 border-b border-border/60 bg-background">
      <div className="flex h-9 items-center gap-1 bg-card/40 px-1.5">
        <Button type="button" variant="ghost" size="icon" onClick={onReload} title="Muat ulang"
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button type="button" variant="ghost" size="sm" title="Port dev server"
          ...
          <DropdownMenuContent align="start" className="max-h-80 min-w-56 overflow-y-auto">
            {PORT_PRESETS.map((p) => (
              <DropdownMenuItem key={p.port} onSelect={(e) => { e.preventDefault(); void tryPort(p.port); }}>
                <span className="flex-1">{p.label}</span>
                <span className="text-xs text-muted-foreground">{checkingPort === p.port ? "memeriksa\u2026" : `:${p.port}`}</span>
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        </DropdownMenu>
        <div className="flex min-w-0 flex-1 items-center">
          <Input ref={inputRef} value={draft} placeholder="http://localhost:3000"
            ...
            onKeyDown={(e) => {
              ...
              else if (e.key === "Escape") { e.preventDefault(); setDraft(url); inputRef.current?.blur(); }
            }}
          />
        </div>
        <Button type="button" variant="ghost" size="icon"
          onClick={() => { if (url) void openUrl(url).catch(console.error); }}
          title="Buka di browser" disabled={!url}
          className="size-7 shrink-0 rounded-md text-muted-foreground hover:bg-accent hover:text-foreground"
        >
...
async function probeUrl(url: string): Promise<boolean> {
  try { await fetch(url, { method: "GET", mode: "no-cors", cache: "no-store", signal: AbortSignal.timeout(900) }); return true; }
  catch { return false; }
}
