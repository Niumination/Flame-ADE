...
async function applyEdits(
  ...
  for (const e of edits) {
    if (e.old_string === e.new_string) {
      return { error: "old_string and new_string are identical", path: abs };
    }
    ...
    if (e.replace_all) {
      ...
      if (n === 0) {
        return { error: `old_string not found: ${JSON.stringify(e.old_string.slice(0, 80))}`, path: abs };
      }
      totalReplacements += n;
    } else {
      ...
      if (second !== -1) {
        return { error: "old_string is not unique. Provide more surrounding context, or set replace_all=true.", path: abs };
      }
  ...
  if (usePlanStore.getState().active) {
    ...
    return { ok: true, replacements: totalReplacements, bytesWritten: content.length, path: abs };
  }
    ...
    edit: tool({
      ...
      execute: async ({ path, old_string, new_string, replace_all }) => {
        ...
        if (!ctx.readCache.has(abs)) {
          return { error: "must call read_file on this path first (read-before-edit invariant).", path: abs };
        }
