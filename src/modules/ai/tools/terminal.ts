  ...
  return {
    suggest_command: tool({
      ...
      inputSchema: z.object({
        command: z.string().describe("The shell command. Single line, no trailing newline."),
        explanation: z.string().optional().describe("Optional one-line note shown beside the command."),
      }),
    ...
    get_terminal_output: tool({
      ...
      execute: async ({ lines }) => {
        if (ctx.isActiveTerminalPrivate()) {
          return { error: "active terminal is in Privacy mode; its buffer is withheld. Ask the user to switch to a regular tab if they want you to see it." };
        }
    ...
    open_preview: tool({
      ...
      inputSchema: z.object({
        url: z.string().describe("Full URL to load (e.g. http://localhost:5173). Must include scheme. Only http/https on loopback hosts are accepted."),
      }),
      execute: async ({ url }) => {
        let parsed: URL;
        try {
        ...
        if (!isLocal) {
          return { error: "open_preview is restricted to localhost URLs. Ask the user to paste the external URL into the preview address bar instead.", url };
        }
