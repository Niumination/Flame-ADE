...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S flame-ade-bin";
    case "debian":
      return `sudo apt install ./FlameADE_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./FlameADE-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update\u2026"
                : manual
                  ? `Flame ADE v${manual.version} is available`
                  : `Flame ADE v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Flame ADE to finish installing."
              : downloading
                ? progress !== null
